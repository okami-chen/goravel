...
func (ss Shadowsocks) ToSurge() string {
	...
	if ss.Plugin == "obfs" {
		text := fmt.Sprintf("%s = ss, %s, %d, encrypt-method=%s, password=%s, obfs=%s, tfo=false, udp-relay=%t",
		...
		return fmt.Sprintf("%s = ss, %s, %d, encrypt-method=%s, password=%s, tfo=false, udp-relay=%t",
...
func (ss Shadowsocks) ToQuantumultX() string {
	...
	text := fmt.Sprintf("shadowsocks = %s:%d, method=%s, password=%s, udp-relay=%t, fast-open=%t",
		ss.Server, ss.Port, ss.Cipher, ss.Password, ss.UDP, ss.UDP)
	if ss.Plugin == "obfs" {
		if ss.PluginOpts["host"].(string) != "" {
			text += ", obfs=" + ss.PluginOpts["mode"].(string) + ", obfs-host=" + ss.PluginOpts["host"].(string)
		}
	}
	text = text + fmt.Sprintf(", tag=%s", ss.Name)
	return text
}
