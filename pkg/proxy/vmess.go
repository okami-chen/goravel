...
func (v Vmess) ToQuantumultX() string {
	...
	if v.WSPath != "" {
		text = text + fmt.Sprintf(", obfs-uri=%s", v.WSPath)
	}

	if v.WSOpts.Headers["Host"] != "" {
		text = text + fmt.Sprintf(", obfs-uri=%s", v.WSOpts.Headers["Host"])
	}

	if !v.SkipCertVerify {
		text = text + fmt.Sprintf(", tls-verification=%t", false)
	}

	if v.AlterID != 0 {
		text = text + fmt.Sprintf(", aead=%t", true)
	} else {
		text = text + fmt.Sprintf(", caead")
	}

	if v.TLS {
		text = text + fmt.Sprintf(", tls-verification=true")
	} else {
		text = text + fmt.Sprintf(", tls-verification=false")
	}

	text = text + fmt.Sprintf(", tag=%s", v.Name)
